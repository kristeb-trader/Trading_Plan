...
> **Reglas:** R-01 a R-09, todas confirmadas por el operador
...
- **Condición medible:**
  - Instrumento de análisis: **NQ** — E-mini Nasdaq-100. $20,00/punto. Tick = 0,25 pts = $5,00.
  - Instrumento de ejecución: **MNQ** — Micro E-mini Nasdaq-100. $2,00/punto. Tick = 0,25 pts = $0,50.
  - Timeframe único: **velas japonesas de 1 minuto**. Ningún otro timeframe toma decisiones.
  - Desfase observado entre NQ y MNQ: **0 a 3 ticks**. Habitualmente 0.
- **Cómo se verifica en NT8:** dos gráficos de 1 minuto abiertos en simultáneo.
  - **Gráfico NQ** → marcado de zonas, lectura de volumen, contexto y estructura.
  - **Gráfico MNQ** → lectura del precio exacto de trigger, stop y target, y envío de la orden.
  - Ningún análisis en MNQ. Ninguna orden desde el gráfico de NQ.
- **Acción:** el nivel de entrada **no se traduce** desde NQ. Se lee el máximo (long) o mínimo (short) real de la vela correspondiente **en el gráfico de MNQ** y se aplica sobre él el desplazamiento de 1 tick. Stop y target se leen igualmente sobre MNQ.
- **Excepciones:** ninguna
- **Ejemplo válido:** `../02_Assets/validos/R-01_valido_01.png` *(pendiente de captura)*
- **Estado:** ✅ **Confirmada**

> **Razón.** El gatillo es de **1 tick**. Un desfase de 2–3 ticks entre NQ y MNQ es hasta 3 veces el gatillo y puede invertir la validez del setup. Además, si el trigger se leyera en MNQ y el stop en NQ, el riesgo calculado no correspondería al instrumento operado y los $120 dejarían de ser $120.
...
- **Cómo se verifica en NT8:** nivel de invalidación marcado sobre el gráfico de NQ; precio leído en MNQ.
- **Acción:** cancelar la orden. El setup queda descartado. **El cupo de R-03 no se consume.** Se puede esperar un nuevo setup sin límite de tiempo dentro de la ventana de R-02.
- **Excepciones:** ninguna
- **Contraejemplo:** `../02_Assets/invalidos/R-04_invalido_01.png` *(pendiente de captura)*
- **Estado:** ✅ **Confirmada**
...
>
> El operador afirmó que la orden siempre se llena. La guía v4 registra el caso contrario (Error 3, 04/05/2026). Con R-07 —orden stop en reposo colocada al cierre de la vela de rompimiento— ese escenario deja de depender de la velocidad de reacción.
...
- **Acción:** ejecutar el primer setup válido; una vez llenada la orden, ignorar el resto de la sesión (R-03).
- **Excepciones:** un setup válido cuya orden caduque sin llenarse (R-04) no consume el cupo ni bloquea los siguientes.
- **Estado:** ✅ **Confirmada**
...
- **Excepciones:** ninguna
- **Ejemplo válido:** `../02_Assets/validos/R-07_valido_01.png` *(pendiente de captura)*
- **Contraejemplo:** `../02_Assets/invalidos/R-07_invalido_01.png` *(pendiente de captura)*
- **Estado:** ✅ **Confirmada**
...
## R-08 · Configuración de ejecución (ATM `K1`)
...
- **Excepciones:** ninguna
- **Ejemplo válido:** `../02_Assets/validos/R-08_valido_01.png` *(pendiente de captura)*
- **Estado:** ✅ **Confirmada**
...
- **Condición medible:** único indicador en pantalla: **Volume Up Down** (NT8), sobre el gráfico de **NQ**. Sin medias móviles, sin osciladores, sin VWAP, sin perfil de volumen, sin herramientas automáticas.
- **Cómo se verifica en NT8:** el umbral de **≥2.000 contratos** se lee sobre la barra de Volume Up Down de la vela de 1 minuto de **NQ**.
- **Acción:** ninguna herramienta adicional puede añadirse al gráfico sin pasar por una revisión de este plan.
- **Excepciones:** ninguna
- **Ejemplo válido:** `../02_Assets/validos/R-09_valido_01.png` *(pendiente de captura)*
- **Estado:** ✅ **Confirmada**
